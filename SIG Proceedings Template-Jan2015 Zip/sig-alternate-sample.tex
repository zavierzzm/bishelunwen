...
This time, client calls server with a single parameter indicating size and server returns the corresponding size string. Results similar to the previous section are shown in Figures \ref{fig:recv}(d-f).

\subsubsection{Analysis}

As is shown in both figures, most of latencies using `\textbf{-p portA:portB}' are no more than 110\% of latencies using `\textbf{--net=host}' in each size group. In fact, when file size becomes much larger, the results are  very close to each other, and the slowdown percentage is nealy zero. Some of the comparison groups even witness a phenomenon where Docker bridge outperforms Docker host. This is because many other factors could slightly change the latency, including CPU scheduling, preempting, network contention, etc. They are outside the scope of this study.
